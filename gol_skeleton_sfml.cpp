...
Say your file is called nextgen.cpp compile it with
  ...
  g++ gol_skeleton_text.cpp nextgen.cpp -o golsfml -lsfml-graphics -lsfml-window -lsfml-system
...
int main(int argc, char* argv[])
{
  ...
  if (argc>1) {
    ...
    int total_ones = 0;
    int percentage_ones = 0;
    double limit = 1/3;
    for (int y=0;y<rows;y++){
      for (int x=0; x<columns;x++){
        srand(time(nullptr));
        bool alive = rand()%2;
        if (alive == 1 && percentage_ones< limit){
          Abuf[y*columns+x] = true;
          total_ones++;
          percentage_ones = total_ones / (rows*columns);
        }
        else{
          Abuf[y*columns+x] = false;
        } 
      }// end for x
    }// end for y
  ...
  while (changes>0 && window.isOpen())
    ...
    //     srand(time(nullptr));
    //     bool alive = rand()%2;
    //     if (alive == 1 && percentage_ones<(1/3)){
    //       Abuf[y*columns+x] = true;
    //       // total_ones++;
    //       // percentage_ones = total_ones / (rows*columns);
    //     }
